		...
		if not self.nextp or not self:has_traction() then
			...
			if d_total >= 2.0 then
				...
				if d_at_speed >= d_total then
					...
					if math.random(1,3) == 1 then minetest.sound_play({name='domob', gain=1.5}, {pos=fp, loop=false}) end
					if math.random(1,100) == 1 then 
						self.object:remove()
						minetest.env:add_entity(fp,"domob:pinkdomob") end
					self.nextp = nil
					self.object:setsprite({x=0, y=0}, 1, 0.5, true)
				else
...
minetest.register_entity("domob:pinkdomob", {
	initial_properties = {
		hp_max = 20,
		physical = true,
		--collisionbox = {-0.4,-1.38,-0.4, 0.4,1.1,0.4},
		...
		if not self.nextp or not self:has_traction() then
			...
			if d_total >= 2.0 then
				...
				if d_at_speed >= d_total then
					...
					if math.random(1,3) == 1 then minetest.sound_play({name='domob', gain=1.5}, {pos=fp, loop=false}) end
					if math.random(1,100) == 1 then 
						minetest.env:add_entity(fp,"domob:domob") end
					self.nextp = nil
					self.object:setsprite({x=0, y=0}, 1, 0.5, true)
				else
					----domob.debug("DM: Travelling")
